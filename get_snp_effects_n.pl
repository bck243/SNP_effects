...
#get N atom flux between reference and new amino acid
...
my $row_number;
my $n_ref_aa; #number of nitrogen atoms in reference aa
my $n_new_aa; #number of nitrogen atoms in new aa
my $n_flux;
#arrays
my @orf_list;
#hashes
...
#print header
print OUT join("\t", ("orf_id", "snp_position", "reference_nucleotide", "first_new_nucleotide", "reference_codon", "new_codon", "snp_position_in_codon","translated_reference_codon", "translated_new_codon", "synonymous", "n_atom_flux"))."\n";
  ...
  while(<IN>) {
      chomp;
      my ($orf_id, $snp_position, $ref_snp_nt, $new_snp_nt) = split(/\t/);
      #$new_snp_nt is a list; just get first item
      ...
      print STDERR "The reference codon is ".$ref_codon."\n";
      print STDERR "The new codon is ".$new_codon."\n";
      ...
      if($new_codon eq "GCT" or $new_codon eq "GCC" or $new_codon eq "GCA" or $new_codon eq "GCG"){
        ...
        print STDERR "The new codon cannot be translated\n";
        $new_aa = "X"; #This is likely because of uncertain nucleotide input from low quality sequences (e.g. "N")
        $n_new_aa = "NA";
      }
      ...
      ##############################
      #Get change in N atoms
      ##############################
      if ($n_new_aa eq "NA" or $n_ref_aa eq "NA"){
        $n_flux = "NA";
      }else{
        $n_flux = $n_new_aa - $n_ref_aa;
      }

      ##############################
      #write out
      ##############################
      #output is: "orf_id", "snp_position", "reference_nucleotide", "first_new_nucleotide", "reference_codon", "new_codon", "snp_position_in_codon","translated_reference_codon", "translated_new_codon", "synonymous", "n_atom_flux"
      print OUT join("\t", ($orf_id, ($snp_position+1), $ref_snp_nt, $new_snp_nt, $ref_codon, $new_codon, $codon_position, $ref_aa, $new_aa, $is_synonymous, $n_flux))."\n";
